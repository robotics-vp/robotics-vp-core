...
- Stage-1 now emits counterfactual/value-target refs and Phase-5 local prep joins them structurally, but there is still no production-grade `CounterfactualEval` substrate for adapt vs no-op / collect-data vs no-op / route A vs B traces.
- Local Economic WM trainer contracts can see value targets, but there is still no dense production supervision target pack for constraint-violation cost, frontier gain from more data, uptime delta, uncertainty-adjusted price tick delta, or rebate deltas.
