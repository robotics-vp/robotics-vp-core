...
- `src/economics/economic_wm_entry.py` and `src/world_model/economic_world_model/scaffold.py` now turn that lower-WM evidence into an Economic WM entry preflight plus deterministic `economic_state_v1`, `allocation_envelope_v1`, and `economic_wm_scaffold_report_v1` artifacts. These artifacts are scaffold-only and explicitly non-promotional.
