...
- `src/economics/economic_wm_entry.py` and `src/world_model/economic_world_model/scaffold.py` now turn that lower-WM evidence into an Economic WM entry preflight plus deterministic `economic_state_v1`, `allocation_envelope_v1`, and `economic_wm_scaffold_report_v1` artifacts. `scripts/run_stage1_pipeline.py` now emits canonical Perception / Grounding, Sim / Synth / Physics, and Embodiment / Actuation state refs at the Stage-1 sidecar boundary, and `src/world_model/economic_world_model/training_rows.py` preserves those refs in local row corpora over Stage-1 proposal admissions. `src/world_model/economic_world_model/allocation_eval.py` produces shadow-only allocation recommendations over those rows, `src/world_model/economic_world_model/evidence_contracts.py` names the evidence requirements for future teacher/provider/GPU/promotion work, and `src/world_model/economic_world_model/provider_runbook.py` compiles those requirements into manifest-shaped run templates. `src/world_model/economic_world_model/provider_runbook_validation.py` validates that those templates remain pending, guarded, launch-denied planning artifacts. `src/world_model/economic_world_model/lower_wm_consumption.py` proves the resulting Economic WM rows carry canonical lower-WM refs directly, and `src/world_model/economic_world_model/neural_architecture_manifest.py` records the future estimator/dynamics/allocator/governance neural topology without weights, GPU runs, or promotion authority. `src/world_model/economic_world_model/resource_surfaces.py` now defines capacity/latency/thermal/battery receipts, companion-compute contracts, degraded-mode runbooks, and queue telemetry ingestion slots; `src/world_model/economic_world_model/phase5_local_prep.py` builds datapack-composition rows, counterfactual/value-target joins, and temporal windows; `scripts/train_economic_world_model_v0.py` emits a shape-checked non-training trainer scaffold; and `src/world_model/economic_world_model/shadow_execution.py` emits advisory work orders plus outcome-comparison slots. `src/world_model/economic_world_model/supervision_substrate.py`, `src/world_model/economic_world_model/shadow_outcomes.py`, and `src/world_model/economic_world_model/lower_wm_maturity_sweep.py` then prove those refs can be re-consumed as typed supervision records, local structural outcome receipts, and lower-WM maturity rows. These artifacts are scaffold/runbook/manifest/shadow-only and explicitly non-promotional.
...
- Stage-1 now emits counterfactual/value-target refs, Phase-5 local prep joins them structurally, and Phase-5.1 materializes them as typed Economic WM supervision records. Still missing: production-grade counterfactual outcome evidence for adapt vs no-op / collect-data vs no-op / route A vs B traces.
- Local Economic WM trainer contracts can see value targets and supervision-record summaries, but dense production target packs for constraint-violation cost, frontier gain from more data, uptime delta, uncertainty-adjusted price tick delta, or rebate deltas still require real outcome receipts.
