...
- Phase 4 now includes a Unitree/G1 bring-up readiness pack in `src/world_model/humanoid_readiness/unitree_bringup_readiness.py` plus `scripts/economic_world_model/prepare_phase4_unitree_bringup_readiness.py`; it emits 9 blocker receipts for dependency inventory, G1Pilot/fallback review, robot asset joint conformance, live-stream contracts, command conformance, timing probe, physical safety preflight, operator e-stop/recovery, and sim/hardware evidence ledger, while keeping ROS2 publish, Unitree SDK2 writes, G1Pilot invocation, honest sim, hardware, live control, reward mutation, training, and promotion denied
- Phase 4 now includes executable local Unitree/G1 harnesses in `src/world_model/humanoid_readiness/unitree_local_harness.py` plus `scripts/economic_world_model/prepare_phase4_unitree_local_harnesses.py`; they emit synthetic low-state / IMU / wireless-e-stop / contact traces, JSONL replay receipts, mock receivers, stale-data validators, Unitree ROS2 message parses, no-publish `LowCmd` / sport-request conformance receipts, mock timing / watchdog receipts, safety/recovery state transitions, and Unitree ROS2 / MuJoCo / G1Pilot preflight receipts while keeping live streams, ROS2/DDS publish, SDK2 writes, G1Pilot invocation, MuJoCo launch, hardware, live control, reward mutation, training, and promotion denied
- Phase 4 now includes a runtime-evidence bridge in `src/world_model/humanoid_readiness/unitree_runtime_bridge.py` plus `scripts/economic_world_model/prepare_phase4_unitree_runtime_evidence_bridge.py`; it emits ROS2/colcon readiness receipts, guarded no-policy MuJoCo headless step receipts, JSONL/rosbag2/MCAP trace adapter receipts, expanded safety-envelope receipts, and scripted operator-recovery drill receipts while keeping ROS2 publish, SDK2 writes, G1Pilot invocation, hardware, live control, reward mutation, training, and promotion denied
- Phase 4 now includes blocker stress probes in `src/world_model/humanoid_readiness/unitree_blocker_probes.py` plus `scripts/economic_world_model/probe_phase4_unitree_blockers.py`; they emit host-toolchain, runtime-import, static source-surface, compile-only DDS/SDK2, multi-model MuJoCo, policy/asset, IsaacLab, LeRobot, trace-module, calibration, and teleop-surface receipts while keeping ROS2 publish, SDK2 writes, G1Pilot invocation, policy control, hardware, live control, reward mutation, training, and promotion denied
...
- As of 2026-05-24, Phase 4 also has a typed local scaffold in `src/world_model/humanoid_readiness/phase4.py`, downstream-controller receipts, and Unitree/G1 bring-up readiness receipts; the current artifact run emits `contract_surface_count=15`, `stub_surface_count=3`, `local_non_hardware_scaffold_complete=true`, `local_downstream_controller_scaffold_complete=true`, `local_pre_purchase_prepared=true`, and `ready_for_phase65_local_meta_nodes=true`, with full deployment closure still blocked on runtime build verification, live streams, control interfaces, timing/jitter traces, companion middleware, operator/recovery drills, physical safety calibration, and hardware or honest sim runtime evidence
- The current Unitree/G1 local harness artifact emits `local_harnesses_complete=true`, `trace_stream_harness_complete=true`, `command_shape_harness_complete=true`, `mock_timing_watchdog_harness_complete=true`, `safety_recovery_harness_complete=true`, and `runtime_preflight_harness_complete=true`; this closes local harness roots only, with `colcon` / `ros2` installation, live stream capture, command echo, physical calibration, recovery drills, and honest sim or hardware traces still blocking deployment evidence
- The current runtime-evidence bridge artifact emits `local_runtime_evidence_bridge_complete=true`, `mujoco_trace_row_count=5`, `minimal_mujoco_headless_step_executed=true`, `trace_ingestion_adapters_complete=true`, `safety_envelope_expansion_complete=true`, and `operator_drill_runner_complete=true`; this narrows the sim blocker to policy-controlled/bridged/deployment-grade runtime evidence, while ROS2/colcon build/import, command echo, real stream import, physical calibration, operator teleop runtime, and DDS/on-robot timing remain blockers
- The current blocker stress-probe artifact emits `local_phase4_probe_expansion_complete=true`, `probe_receipt_count=14`, `succeeded_probe_count=8`, `blocked_probe_count=6`, `mujoco_model_stress_receipt_count=5`, `mujoco_model_stress_success_count=5`, `g1pilot_static_surface_succeeded=true`, `cyclonedds_header_compile_succeeded=true`, `policy_checkpoint_visible=true`, `isaaclab_task_surface_visible=true`, and `lerobot_adapter_surface_visible=true`; this turns successful probes into typed evidence while preserving missing ROS2/colcon and `/opt/ros`, Linux SDK2 compile/runtime, rosbag2/MCAP modules/files, calibration sidecars, G1Pilot runtime dependencies, command echo, teleop runtime, DDS/network timing, and policy-controlled traces as blockers
