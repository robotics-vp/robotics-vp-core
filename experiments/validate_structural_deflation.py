...
def validate_surplus_decomposition(df, tolerance=1e-4):
    ...
    df_spread["decomp_error"] = np.abs(
        df_spread["decomp_lhs"] - df_spread["decomp_rhs"]
    )
...
def validate_productivity_incentive(df):
    ...
    if len(df_spread) < 10:
        return {
            "n_episodes": len(df_spread),
            "message": "Insufficient episodes with spread",
        }
...
def plot_validation(df, output_path="plots/structural_deflation_validation.png"):
    ...
    fig.suptitle(
        "Structural Deflation Theorem: Empirical Validation",
        fontsize=14,
        fontweight="bold",
    )
    ...
    ax = axes[0, 0]
    ax.plot(
        df["episode"], df["consumer_surplus"], alpha=0.6, linewidth=0.8, label="CS(t)"
    )
    ...
    ax.text(
        0.02,
        0.98,
        f"Violations: {violations}/{len(df)} ({100 * violations / len(df):.1f}%)",
    ...
    if len(df_spread) > 0:
        ...
        ax.text(
            0.5,
            0.5,
            "No episodes with positive spread",
            transform=ax.transAxes,
            ha="center",
            va="center",
            fontsize=12,
    ...
    ax = axes[1, 1]
    ax.plot(
        df["episode"], df["rho"], alpha=0.4, linewidth=0.8, label="ρ(t) = MC_r/MC_h"
    )
    ...
    ax.fill_between(
        ...
        alpha=0.2,
        color="green",
        label="Structural deflation zone",
    )
...
def generate_summary(
    ...
    lines.append("-" * 80)
    lines.append("Claim: CS(t) ≥ 0 for all t")
    lines.append("")
    lines.append("Validation:")
    lines.append(
        f"  Valid episodes: {len(df) - cs_result['n_violations']}/{len(df)} ({cs_result['pct_valid']:.1f}%)"
    )
    lines.append(f"  Violations: {cs_result['n_violations']}")
    if cs_result["n_violations"] == 0:
        lines.append("  ✅ GUARANTEE HOLDS for ALL episodes")
    else:
        lines.append(f"  ⚠️  {cs_result['n_violations']} violations detected")
    lines.append("")
    lines.append("Consumer Surplus Statistics:")
    ...
    lines.append("-" * 80)
    lines.append("Claim: SV(t) = R(t) + C(t) for episodes with positive spread")
    lines.append("")
    if decomp_result.get("n_episodes", 0) > 0:
        lines.append("Validation:")
        lines.append(f"  Episodes with spread: {decomp_result['n_episodes']}")
        lines.append(
            f"  Valid decompositions: {decomp_result['n_episodes'] - decomp_result['n_violations']}/{decomp_result['n_episodes']} ({decomp_result['pct_valid']:.1f}%)"
        )
        lines.append(f"  Violations: {decomp_result['n_violations']}")
        if decomp_result["n_violations"] == 0:
            lines.append("  ✅ EXACT DECOMPOSITION for ALL episodes with spread")
        else:
            lines.append(f"  ⚠️  {decomp_result['n_violations']} violations detected")
        lines.append("")
        lines.append("Decomposition Error:")
        lines.append(f"  Mean: ${decomp_result['mean_error']:.6f}")
        lines.append(f"  Max:  ${decomp_result['max_error']:.6f}")
    else:
        lines.append("  No episodes with positive spread (robot not yet beating human)")
    lines.append("")
    ...
    lines.append("-" * 80)
    lines.append("Claim: C(t) increases when MPL_r(t) increases (above parity)")
    lines.append("")
    if incentive_result.get("n_episodes", 0) >= 10:
        lines.append("Validation:")
        lines.append(f"  Episodes analyzed: {incentive_result['n_episodes']}")
        lines.append(
            f"  Pearson correlation: r = {incentive_result['pearson_r']:.4f} (p = {incentive_result['pearson_p']:.2e})"
        )
        lines.append(
            f"  Spearman correlation: ρ = {incentive_result['spearman_r']:.4f} (p = {incentive_result['spearman_p']:.2e})"
        )
        lines.append(
            f"  Linear regression slope: {incentive_result['linear_slope']:.4f}"
        )
        if incentive_result["positive_correlation"]:
            lines.append("  ✅ POSITIVE CORRELATION confirmed (p < 0.05)")
        else:
            lines.append("  ⚠️  Correlation not significant or negative")
    else:
        lines.append(
            f"  Insufficient episodes with spread ({incentive_result.get('n_episodes', 0)} < 10)"
        )
    lines.append("")
    ...
    lines.append("-" * 80)
    lines.append("Claim: ρ(t) = MC_r(t)/MC_h(t) ≤ 1 and decreasing over time")
    lines.append("")
    lines.append("Validation:")
    lines.append(
        f"  Episodes with ρ ≤ 1: {len(df) - deflation_result['n_violations_ceiling']}/{len(df)} ({deflation_result['pct_valid_ceiling']:.1f}%)"
    )
    lines.append(f"  Ceiling violations: {deflation_result['n_violations_ceiling']}")
    if deflation_result["n_violations_ceiling"] == 0:
        lines.append("  ✅ CEILING HOLDS: ρ ≤ 1 for ALL episodes")
    else:
        lines.append(
            f"  ⚠️  {deflation_result['n_violations_ceiling']} episodes above ceiling"
        )
    lines.append("")
    lines.append("Deflation Ratio ρ(t):")
    ...
    lines.append(
        f"  Linear slope (raw): {deflation_result['linear_slope']:.2e} per episode"
    )
    if deflation_result["linear_slope_ma"] is not None:
        lines.append(
            f"  Linear slope (MA):  {deflation_result['linear_slope_ma']:.2e} per episode"
        )
    if deflation_result["is_decreasing"]:
        lines.append("  ✅ DECREASING TREND confirmed (negative slope)")
    else:
        lines.append("  ⚠️  Trend not decreasing (positive slope)")
    lines.append("")
    ...
    all_pass = (
        cs_result["n_violations"] == 0
        and (
            decomp_result.get("n_violations", 0) == 0
            or decomp_result.get("n_episodes", 0) == 0
        )
        and incentive_result.get("positive_correlation", False)
        and deflation_result["n_violations_ceiling"] == 0
        and deflation_result["is_decreasing"]
    )
    ...
    lines.append(
        "  1. Customer protection (CS ≥ 0): "
        + ("✅" if cs_result["n_violations"] == 0 else "⚠️")
    )
    lines.append(
        "  2. Fair value split (SV = R + C): "
        + ("✅" if decomp_result.get("n_violations", 0) == 0 else "⚠️")
    )
    lines.append(
        "  3. Aligned incentives (C ↑ when MPL ↑): "
        + ("✅" if incentive_result.get("positive_correlation", False) else "⚠️")
    )
    lines.append(
        "  4. Structural deflation (ρ ≤ 1, decreasing): "
        + ("✅" if deflation_result["is_decreasing"] else "⚠️")
    )
    lines.append("")
    lines.append("=" * 80)
...
def main():
    ...
    if decomp_result.get("n_episodes", 0) > 0:
        print(
            f"✓ Theorem 2 (Surplus Decomposition): {decomp_result['pct_valid']:.1f}% valid"
        )
    else:
        print("✓ Theorem 2 (Surplus Decomposition): No episodes with spread yet")

    incentive_result = validate_productivity_incentive(df)
    if incentive_result.get("n_episodes", 0) >= 10:
        print(
            f"✓ Theorem 3 (Productivity Incentive): r={incentive_result['pearson_r']:.3f}, p={incentive_result['pearson_p']:.2e}"
        )
    else:
        print("✓ Theorem 3 (Productivity Incentive): Insufficient data")

    deflation_result = validate_structural_deflation(df)
    print(
        f"✓ Theorem 4 (Structural Deflation): ρ {deflation_result['initial_rho']:.3f} → {deflation_result['final_rho']:.3f}"
    )
