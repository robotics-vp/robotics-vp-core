...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--image-name", type=str, default="", help="Container image already containing the desired repo state")
    parser.add_argument("--template-id", type=str, default="", help="Runpod template id to launch instead of an explicit image")
    parser.add_argument("--gpu-type", type=str, default="", help="Exact Runpod GPU type string visible to runpodctl")
    ...
    parser.add_argument("--cost", type=float, default=0.0, help="Optional Runpod create-pod cost ceiling in USD/hour")
    ...
    parser.add_argument("--port", action="append", default=[], help="Ports to expose, e.g. 8888/http")
    parser.add_argument("--force", action="store_true", help="Launch even when the readiness gates are not satisfied")
    parser.add_argument("--dry-run", action="store_true", help="Write the launch plan but do not call runpodctl")
    parser.add_argument(
        "--receipt-dir",
        type=str,
        default="",
...
def _resolve_defaults(args: argparse.Namespace, selected: Dict[str, object]) -> Dict[str, object]:
    runpod_defaults = dict(selected.get("recommended_runpod", {}) or {})
    return {
        "gpu_type": args.gpu_type or str(runpod_defaults.get("gpu_label", "")),
        "gpu_count": args.gpu_count or int(runpod_defaults.get("gpu_count", 1) or 1),
        "container_disk_gb": args.container_disk_gb or int(runpod_defaults.get("container_disk_gb", 50) or 50),
        "volume_gb": args.volume_gb or int(runpod_defaults.get("volume_gb", 100) or 100),
        "hourly_price_usd": float(runpod_defaults.get("hourly_price_usd", 0.0) or 0.0),
    }


def _build_remote_command(bundle_id: str, run_id: str, force: bool, network_volume_id: str) -> str:
...
def main() -> None:
    ...
    if not defaults["gpu_type"]:
        raise SystemExit("GPU type is empty. Pass --gpu-type with the exact Runpod GPU type string.")

    run_id = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ") + f"_{selected['bundle_id']}"
    local_receipt_dir = Path(args.receipt_dir) if args.receipt_dir else REPO_ROOT / "artifacts" / "runpod_training" / run_id
    ...
    _write_text(local_receipt_dir / "launch_plan.json", json.dumps(launch_plan, indent=2, sort_keys=True))
    ...
    _write_text(local_receipt_dir / "launch_receipt.json", json.dumps(receipt, indent=2, sort_keys=True))
