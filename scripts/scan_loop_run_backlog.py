#!/usr/bin/env python3
"""Evaluate and optionally execute ready loop-run backlog items."""
...
from src.orchestrator.loop_run_backlog import (  # noqa: E402
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Scan the loop-run backlog and optionally execute ready entries.")
    parser.add_argument("--backlog", default=str(DEFAULT_LOOP_RUN_BACKLOG_PATH))
    parser.add_argument("--output-json", default="")
    parser.add_argument("--execute-ready", action="store_true")
    parser.add_argument("--include-manual", action="store_true", help="Also execute ready runs with auto_trigger=false.")
    args = parser.parse_args()

    host = collect_host_capabilities()
    assessments = evaluate_loop_run_backlog(
