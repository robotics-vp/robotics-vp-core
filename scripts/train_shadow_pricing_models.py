#!/usr/bin/env python3
"""Train learned shadow pricing/value/support models under the canonical runtime."""
...
from src.learning.data_value_models import DataValueModel, train_data_value_model
from src.learning.pricing_models import PricingDeltaModel, train_pricing_delta_model
from src.learning.regal_support_models import RegalSupportModel, train_regal_support_model
...
def _select_episode_ids(dataset, args: argparse.Namespace, output_root: Path, receipt_label_dir: str) -> tuple[list[str], Dict[str, str], Dict[str, Any], Dict[str, Any]]:
...
def _run_training(args: argparse.Namespace, runner: Optional[RegalTrainingRunner]) -> Dict[str, Any]:
    ...
    episodes = [row for row in dataset.episodes if row.episode_id in selected_set] if selected_set else list(dataset.episodes)
    ...
    report = build_promotion_evidence_report(
        ...
        node_ids=("pricing_truth_regal", "data_value_regal", "regal_support_advisor"),
        evidence_pointers={
    ...
    if runner is not None:
        ...
        runner.set_sampler_config(seed=seed, config_sha=summary["config_digest"])
        ...
        runner.update_step(epochs * max(1, len(episodes)))
        ...
        runner.register_artifact("shadow_model_train_summary", summary_path)
        runner.register_checkpoint(
            build_checkpoint_record(
                checkpoint_id="pricing_delta",
                model_family="shadow_pricing_delta",
                model_version=PricingDeltaModel.model_version,
                path=pricing_path,
                epoch=epochs,
                step=epochs * max(1, len(episodes)),
                metadata=pricing_metrics,
            )
        )
        runner.register_checkpoint(
            build_checkpoint_record(
                checkpoint_id="data_value",
                model_family="shadow_data_value",
        ...
        runner.register_checkpoint(
            build_checkpoint_record(
                checkpoint_id="regal_support",
                model_family="shadow_regal_support",
                model_version=RegalSupportModel.model_version,
                path=regal_path,
                epoch=epochs,
                step=epochs * max(1, len(episodes)),
                metadata=regal_metrics,
            )
        )
    ...
    payload = {
        ...
        "artifacts": {
            ...
            **queue_paths,
            **promotion_paths,
            "shadow_model_train_summary": str(summary_path),
        },
    }
