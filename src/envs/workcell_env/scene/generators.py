...
from src.envs.workcell_env.scene.parts_library import (
    generate_part_spec,
...
class WorkcellSceneGenerator:
    ...
    def generate(
        self, config: WorkcellEnvConfig, seed: Optional[int] = None
    ) -> WorkcellSceneSpec:
        ...
        containers, container_regions = self._generate_containers(
            config, rng, bounds, placements
        )
    ...
    def _generate_stations(
        ...
        anchors = self._station_anchors(
            config.num_stations, config.topology_type, bounds
        )
        for idx, anchor in enumerate(anchors):
            position = self._place_near(
                anchor, rng, bounds, placements, radius_m=0.5, jitter_m=(0.2, 0.2)
            )
    ...
    def _generate_fixtures(
        ...
        for idx in range(config.num_fixtures):
            ...
            position = self._place_near(
                anchor, rng, bounds, placements, radius_m=radius_m, jitter_m=(0.3, 0.3)
            )
            fixtures.append(
                generate_fixture_spec(
    ...
    def _generate_containers(
        ...
        for idx, anchor in enumerate(anchors):
            ...
            position = self._place_near(
                anchor, rng, bounds, placements, radius_m=radius_m, jitter_m=(0.2, 0.2)
            )
            containers.append(
                ContainerSpec(
                    id=f"bin_{idx}",
    ...
    def _generate_parts(
        ...
        num_conveyor_parts = (
            min(len(conveyor_regions) * 3, config.num_parts // 3)
            if conveyor_regions
            else 0
        )
        ...
        for idx in range(config.num_parts):
            part_type = (
                rng.choice(list(config.part_types)) if config.part_types else None
            )
            ...
            position = self._place_in_region(
                region, rng, bounds, part_positions, radius_m
            )
            parts.append(
                generate_part_spec(
                    part_id=f"part_{idx}",
    ...
    def _within_bounds(
        ...
        return (
            abs(x) + radius_m <= bounds[0] / 2.0
            and abs(y) + radius_m <= bounds[1] / 2.0
        )
        ...
        while x <= x_max:
            ...
            while y <= y_max:
                if self._within_bounds(
                    x, y, radius_m, bounds
                ) and self._is_collision_free(x, y, radius_m, placements):
                    return x, y
                y += step
            x += step
        return None
