...
def load_orchestrator_shell_policy_runtime_package(
    path: str | Path,
...
class LoadedOrchestratorShellPolicyHelper:
    def __init__(self, package: OrchestratorShellPolicyRuntimePackage) -> None:
        if not TORCH_AVAILABLE:
            raise ImportError(
                "PyTorch is required to load the orchestrator shell policy helper"
            )
        checkpoint_path = Path(package.checkpoint_path)
        if not checkpoint_path.exists():
            raise FileNotFoundError(
                f"orchestrator shell policy checkpoint not found: {checkpoint_path}"
            )
        payload = torch.load(
        ...
        self.model = OrchestratorShellPolicyNet(
    ...
    def apply_to_advisory(
        ...
        *,
        ...
        feature_vector = build_shell_policy_feature_vector(
            snapshot, trust_matrix=trust_matrix
        )
        tensor = torch.from_numpy(feature_vector).float().unsqueeze(0)
        with torch.no_grad():
            preset_logits, strategy_logits, safety_logits, activation_logits = (
                self.model(tensor)
            )
        ...
        helper_blend = dict(
            self.inference_contract.get("helper_blend_policy", {}) or {}
        )
        helper_weight = float(
            helper_blend.get(
        ...
        max_safety_delta = float(
            helper_blend.get(
                "promoted_max_safety_delta"
                if benchmark_gate_ready
                else "shadow_candidate_max_safety_delta",
        ...
        prior_safety = _clamp01(
            float(heuristic_advisory.get("safety_emphasis", 0.0) or 0.0)
        )
        learned_preset_distribution = _normalize_vector(
            preset_probs, SHELL_POLICY_PRESET_LABELS
        )
        learned_strategy_distribution = _normalize_vector(
            strategy_probs, SHELL_POLICY_STRATEGY_KEYS
        )
        ...
        blended_preset_distribution = _normalize_vector(
            np.asarray(
                [
                    blended_preset_distribution[label]
                    for label in SHELL_POLICY_PRESET_LABELS
                ],
                dtype=np.float64,
            ),
        ...
        blended_strategy_distribution = _normalize_vector(
            np.asarray(
                [
                    blended_strategy_distribution[label]
                    for label in SHELL_POLICY_STRATEGY_KEYS
                ],
                dtype=np.float64,
            ),
        ...
        raw_safety = ((1.0 - helper_weight) * prior_safety) + (
            helper_weight * safety_score
        )
        blended_safety = _clamp01(
            min(
                max(raw_safety, prior_safety - max_safety_delta),
                prior_safety + max_safety_delta,
            )
        )

        execution_mode = str(
            heuristic_advisory.get("execution_mode", "advisory") or "advisory"
        )
        ...
        if benchmark_gate_ready and execution_mode != "advisory":
            if activation_score < 0.3 and helper_mode == "required":
                ...
                activation_plan["helper_recommended_mode"] = (
                    execution_mode if activation_score >= 0.5 else "advisory"
                )

        return {
            "focus_objective_presets": _focus_presets_from_distribution(
                blended_preset_distribution
            ),
