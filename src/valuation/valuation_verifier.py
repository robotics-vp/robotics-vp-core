...
def _load_manifest(manifest_path: Path) -> Tuple[Optional[RunManifestV1], Optional[str]]:
    """Load and validate run_manifest."""
...
def verify_run(output_dir: str) -> VerificationReportV1:
    ...
    for file_path, name in [
        ...
        checks.append(
            VerificationCheckV1(
                ...
                message=f"Required file {name} exists"
                if file_path.exists()
                else f"Missing required file: {name}",
            )
        )
    ...
    if manifest is None:
        return _build_report("unknown", checks, warnings, None, 0)
    run_manifest: RunManifestV1 = manifest

    run_id = run_manifest.run_id

    # Detect training run early (needed for multiple checks including 10c, 11, 15)
    is_training_run = (
        run_manifest.final_weights_sha is not None
        and run_manifest.baseline_weights_sha is not None
        and run_manifest.final_weights_sha != run_manifest.baseline_weights_sha
    )
    ...
    if plan_path.exists():
        stored_sha = run_manifest.plan_sha
    ...
    for record in ledger_records:
        ...
        if record.regal_degraded:
            ...
            checks.append(
                VerificationCheckV1(
                    ...
                    message="regal_degraded=True → allow_deploy=False"
                    if not record.allow_deploy
            ...
            checks.append(
                VerificationCheckV1(
                    ...
                    passed=not record.allow_deploy,
                    message="regal.all_passed=False → allow_deploy=False"
                    if not record.allow_deploy
        ...
        if record.regal and record.regal.reports:
            ...
            if run_manifest.regal_report_sha:
                checks.append(
                    VerificationCheckV1(
    ...
    if run_manifest.regal_config_sha and not run_manifest.trajectory_audit_sha:
        warnings.append(
            "regal_config_sha present but trajectory_audit_sha missing (may be intentional if no training)"
        )

    # 7. Verify quarantine manifest if present
    if run_manifest.quarantine_manifest_sha:
        checks.append(
            VerificationCheckV1(
                check_id="quarantine_manifest_sha_present",
                passed=True,
                message=f"Quarantine manifest SHA recorded: {run_manifest.quarantine_manifest_sha[:16]}",
            )
        )

    # 8. Verify deploy_gate_decision_sha if present and matches allow_deploy
    if run_manifest.deploy_gate_decision_sha:
        checks.append(
            VerificationCheckV1(
                check_id="deploy_gate_decision_sha_present",
                passed=True,
                message=f"Deploy gate decision SHA recorded: {run_manifest.deploy_gate_decision_sha[:16]}",
            )
        )

    # 9. Verify orchestrator_state_sha presence (Phase 1)
    if run_manifest.orchestrator_state_sha:
        ...
        if orchestrator_state_path.exists():
            ...
            checks.append(
                VerificationCheckV1(
                    ...
                    passed=run_manifest.orchestrator_state_sha == computed_sha,
                    message="Orchestrator state SHA matches"
                    if run_manifest.orchestrator_state_sha == computed_sha
                    else "Orchestrator state SHA mismatch",
                    expected=run_manifest.orchestrator_state_sha,
                    actual=computed_sha,
                )
            )
        else:
            checks.append(
                VerificationCheckV1(
                    ...
                    passed=True,
                    message=f"Orchestrator state SHA recorded: {run_manifest.orchestrator_state_sha[:16]}",
                )
            )

    # 10. Verify selection_manifest_sha presence (Phase 2)
    if run_manifest.selection_manifest_sha:
        ...
        if selection_manifest_path.exists():
            ...
            checks.append(
                VerificationCheckV1(
                    ...
                    passed=run_manifest.selection_manifest_sha == computed_sha,
                    message="Selection manifest SHA matches"
                    if run_manifest.selection_manifest_sha == computed_sha
                    else "Selection manifest SHA mismatch",
                    expected=run_manifest.selection_manifest_sha,
                    actual=computed_sha,
                )
            )
            ...
            checks.append(
                VerificationCheckV1(
                    ...
                    passed=True,
                    message=f"Selection manifest SHA recorded: {run_manifest.selection_manifest_sha[:16]}",
                )
            )
    ...
    if is_training_run:
        checks.append(
            VerificationCheckV1(
                ...
                passed=run_manifest.trajectory_audit_sha is not None,
                message="Training run has trajectory_audit_sha"
                if run_manifest.trajectory_audit_sha
    ...
    if run_manifest.probe_report_sha and run_manifest.regal_context_sha:
                ...
                if probe_regal_ctx:
                    checks.append(
                        VerificationCheckV1(
                            ...
                            passed=probe_regal_ctx == run_manifest.regal_context_sha,
                            message="Probe report regal_context_sha matches manifest"
                            if probe_regal_ctx == run_manifest.regal_context_sha
                            else "Probe report regal_context_sha mismatch",
                            expected=run_manifest.regal_context_sha,
    ...
    if run_manifest.deploy_gate_inputs_sha:
        ...
        if deploy_gate_path.exists():
            ...
            checks.append(
                VerificationCheckV1(
                    ...
                    passed=run_manifest.deploy_gate_inputs_sha == computed_sha,
                    message="Deploy gate inputs SHA matches"
                    if run_manifest.deploy_gate_inputs_sha == computed_sha
                    else "Deploy gate inputs SHA mismatch",
                    expected=run_manifest.deploy_gate_inputs_sha,
                    actual=computed_sha,
                )
            )
    ...
    if run_manifest.verification_report_sha:
        ...
        if verification_report_path.exists():
            ...
            checks.append(
                VerificationCheckV1(
                    ...
                    passed=run_manifest.verification_report_sha == computed_sha,
                    message="Verification report SHA matches"
                    if run_manifest.verification_report_sha == computed_sha
                    else "Verification report SHA mismatch (may need re-run)",
                    expected=run_manifest.verification_report_sha,
                    actual=computed_sha,
                )
            )
    ...
    if run_manifest.trajectory_audit_sha and trajectory_audit_path.exists() and is_training_run:
        try:
        ...
        run_manifest.trajectory_audit_sha and trajectory_audit_path.exists() and not is_training_run
    ):
        # Non-training runs: just warn if missing, don't fail
        warnings.append(
            "trajectory_audit present but not a training run; skipping strict RewardBreakdown check"
        )

    # 16. Verify ledger_regal_sha if present (Phase 8: file-verified regal)
    if getattr(run_manifest, "ledger_regal_sha", None):
        ...
        if ledger_regal_path.exists():
            ...
            checks.append(
                VerificationCheckV1(
                    ...
                    passed=run_manifest.ledger_regal_sha == computed_sha,
                    message="Ledger regal SHA matches"
                    if run_manifest.ledger_regal_sha == computed_sha
                    else "Ledger regal SHA mismatch",
                    expected=run_manifest.ledger_regal_sha,
                    actual=computed_sha,
                )
            )
        else:
        ...
        try:
            ...
            checks.append(
                VerificationCheckV1(
                    ...
                    message="Orchestrator counters valid (all non-negative)"
                    if counters_valid
            ...
            if is_training_run:
                ...
                checks.append(
                    VerificationCheckV1(
                        ...
                        passed=has_deltas,
                        message=f"Training run has applied_knob_deltas ({len(applied_deltas or [])} deltas)"
                        if has_deltas
    ...
    if applied_deltas_path.exists() and orchestrator_state_path.exists():
        try:
            ...
            file_sha = sha256_json(
                file_deltas if isinstance(file_deltas, list) else file_deltas.get("deltas", [])
            )
            ...
            checks.append(
                VerificationCheckV1(
                    ...
                    message="Applied knob deltas match orchestrator state"
                    if file_sha == orch_sha
    ...
    ledger_regal_path = output_path / "ledger_regal.json"
    if is_training_run:
        has_ledger_regal = ledger_regal_path.exists() and bool(
            getattr(run_manifest, "ledger_regal_sha", None)
        )
        checks.append(
            VerificationCheckV1(
                check_id="ledger_regal_required_for_full",
                passed=has_ledger_regal,
                message="FULL run has ledger_regal.json with SHA"
                if has_ledger_regal
    ...
    if orchestrator_state_path.exists() and plan_applied_events_path.exists():
        try:
            ...
            with open(plan_applied_events_path, "r") as f:
                for line in f:
                    if line.strip():
                        plan_events.append(json.loads(line.strip()))
            ...
            checks.append(
                VerificationCheckV1(
                    ...
                    message="Orchestrator decisions consistent with plan_applied_events"
                    if consistency_ok
        ...
        try:
            ...
            checks.append(
                VerificationCheckV1(
                    ...
                    passed=eligible_ok,
                    message=f"Eligible count healthy: {len(eligible_ids)}"
                    if eligible_ok
            ...
            if total_considered > 0:
                ...
                checks.append(
                    VerificationCheckV1(
                        ...
                        passed=rejection_ok,
                        message=f"Rejection ratio normal: {rejection_ratio:.1%}"
                        if rejection_ok
    ...
    if econ_tensor_path.exists() and is_training_run:
        try:
            ...
            evidence_present = (
                all(evidence.get(k) is not None for k in required_evidence) if evidence else False
            )
            ...
            checks.append(
                VerificationCheckV1(
                    ...
                    message="Econ tensor has evidence_summary"
                    if evidence_present
    ...
    if getattr(run_manifest, "regality_thresholds_sha", None):
        if regality_thresholds_path.exists():
            computed_sha = sha256_file(str(regality_thresholds_path))
            checks.append(
                VerificationCheckV1(
                    check_id="regality_thresholds_sha_match",
                    passed=run_manifest.regality_thresholds_sha == computed_sha,
                    message="Regality thresholds SHA matches"
                    if run_manifest.regality_thresholds_sha == computed_sha
                    else "Regality thresholds SHA mismatch",
                    expected=run_manifest.regality_thresholds_sha,
                    actual=computed_sha,
                    severity="FAIL",
                )
            )
    ...
    manifest_sha = sha256_json(run_manifest.model_dump(mode="json"))
