...
def fuse_feature_pyramid(
    ...
    normalized_weights = {
        k: max(epsilon, float(v)) / total_weight for k, v in base_weights.items()
    }
    ...
    if not use_neural or not TORCH_AVAILABLE:
        ...
        return _fuse_pyramid_numpy(
            pyramid, keys, normalized_weights, feature_dim, epsilon
        )
        ...
        def run_fusion(*args):
            ...
            for i in range(len(sorted_keys) - 1, -1, -1):
                ...
                if i == len(sorted_keys) - 1:
                    ...
                    w_sum = w1 + w2 + epsilon
                    top_down[level] = (
                        w1 * input_map[level] + w2 * top_down[finer_level]
                    ) / w_sum
            ...
            for i in range(len(sorted_keys)):
                ...
                if i == 0:
                    ...
                    w_sum = w1 + w2 + epsilon
                    bottom_up[level] = (
                        w1 * input_map[level] + w2 * top_down[level]
                    ) / w_sum
                else:
...
def _fuse_pyramid_numpy(
    ...
    for k in keys:
        ...
        if len(keys) > 1:
            neighbors = [
                np.asarray(pyramid[n], dtype=np.float32).flatten()[:feature_dim]
                for n in keys
                if n != k
            ]
            if neighbors:
    ...
    def _fuse_pyramid_neural(
        ...
        for i in range(len(keys) - 1, -1, -1):
            ...
            if i == len(keys) - 1:
                ...
                top_down[level] = (
                    w1 * features[level] + w2 * top_down[finer_level]
                ) / w_sum
