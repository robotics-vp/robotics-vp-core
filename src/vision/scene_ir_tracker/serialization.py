...
from src.vision.scene_ir_tracker.types import SceneTracks
...
def serialize_scene_tracks_v1(
    ...
    for track_id, track_history in scene_tracks.tracks.items():
        if track_history:
            ...
            entity_types[k] = (
                ENTITY_TYPE_BODY
                if first_entity.entity_type == "body"
    ...
    z_shape_arr = (
        np.zeros((T, K, z_shape_dim), dtype=latent_dtype) if z_shape_dim > 0 else None
    )
    z_tex_arr = (
        np.zeros((T, K, z_tex_dim), dtype=latent_dtype) if z_tex_dim > 0 else None
    )
    ...
    result["scene_tracks_v1/summary_json"] = np.array(
        [json.dumps(summary)], dtype="U1024"
    )
...
def get_scene_ir_summary_dict(scene_tracks: SceneTracks) -> Dict[str, Any]:
    ...
    return {
        ...
        "ir_loss_std": float(np.std(metrics.ir_loss_per_frame))
        if metrics.ir_loss_per_frame
        else 0.0,
        "id_switch_count": int(metrics.id_switch_count),
        "id_switch_rate": float(
            metrics.id_switch_count / max(metrics.total_frames, 1) * 100
        ),
        "occlusion_rate": float(metrics.occlusion_rate),
        "convergence_rate": float(
            metrics.converged_count / max(metrics.total_frames, 1)
        ),
