...
def load_feedback_topology_runtime_package(
    path: str | Path,
...
def resolve_feedback_adapter_helper(
    ...
    *,
    ...
    if helper is None:
        if mode == "required":
            raise ValueError(
                "feedback-adapter mode 'required' but no helper was provided"
            )
        return None, {
            "mode": mode,
    ...
    if isinstance(helper, SemanticFeedbackAdapterPackage):
        ...
        if mode == "required" and not benchmark_gate_ready:
            raise ValueError(
                "feedback-adapter mode 'required' requires a benchmark-gated package"
            )
        return helper, {
            "mode": mode,
    ...
    if checkpoint_path is None or not checkpoint_path.exists():
        if mode == "required":
            raise ValueError(
                "feedback-adapter mode 'required' but no loadable checkpoint/package was found"
            )
        return None, {
            "mode": mode,
            "status": "package_missing",
    ...
    if torch is None:
        raise ImportError(
            "PyTorch is required to load the semantic feedback adapter helper"
        )
