"""Runtime package loading for semantic WM refiner helpers."""
...
from src.world_model.semantic_wm_refiner import SemanticWMRefinerPackage
...
class SemanticWMRefinerRuntimePackage:
    package_id: str
    package_path: str
    checkpoint_path: str
...
def load_semantic_wm_refiner_runtime_package(
    ...
    return SemanticWMRefinerRuntimePackage(
...
def resolve_semantic_wm_refiner_helper(
    helper: Any,
    *,
    mode: Literal["disabled", "auto", "required"] = "auto",
) -> tuple[Optional[SemanticWMRefinerPackage], Dict[str, Any]]:
    if mode not in {"disabled", "auto", "required"}:
        raise ValueError(f"Unsupported semantic-WM-refiner mode: {mode}")
    if mode == "disabled":
        return None, {
            "mode": mode,
            "status": "disabled",
    ...
    package: Optional[SemanticWMRefinerRuntimePackage] = None
    checkpoint_path: Optional[Path] = None
    if isinstance(helper, (str, Path)):
        candidate = Path(helper)
        if candidate.suffix == ".json":
            package = load_semantic_wm_refiner_runtime_package(candidate)
            checkpoint_path = Path(package.checkpoint_path)
        else:
            checkpoint_path = candidate
    elif isinstance(helper, Mapping):
        if "checkpoint_path" in helper:
            package = SemanticWMRefinerRuntimePackage(
                package_id=str(helper.get("package_id", "semantic_wm_refiner")),
            ...
            checkpoint_path = Path(package.checkpoint_path)
        elif "object_state_dict" in helper:
            loaded = SemanticWMRefinerPackage.from_checkpoint(helper)
            return loaded, {
                "mode": mode,
...
__all__ = [
    "SemanticWMRefinerRuntimePackage",
    "load_semantic_wm_refiner_runtime_package",
