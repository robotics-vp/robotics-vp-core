"""Consumer selection over Isaac/Unitree executable-adapter requests."""
...
EXTERNAL_CONSUMER_MODES = {
    "sim_eval": "external_sim_launch",
    "teleop_bridge": "external_teleop_bridge",
    "lerobot_eval": "external_lerobot_eval",
    "physical_deploy": "external_physical_deploy",
}


def build_isaac_unitree_executable_adapter_consumer(
    ...
    supports_local_python_bridge = bool(request.get("supports_local_python_bridge", False))
    missing_preconditions = strings(request.get("missing_preconditions"))
    command = str(request.get("command", "") or "")
    cwd = str(request.get("cwd", "") or "")

    uses_local_python_bridge = deployment_mode == "sim_eval" and supports_local_python_bridge
    consumer_mode = (
        "local_python_bridge"
        if uses_local_python_bridge
        else EXTERNAL_CONSUMER_MODES.get(deployment_mode, "external_runtime_launch")
    )
    if not uses_local_python_bridge:
        if not command:
    ...
    payload = {
        ...
        "uses_local_python_bridge": uses_local_python_bridge,
        "external_runtime_required": not uses_local_python_bridge,
...
__all__ = ["build_isaac_unitree_executable_adapter_consumer"]
