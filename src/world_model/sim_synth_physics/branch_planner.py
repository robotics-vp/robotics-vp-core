...
try:
    ...
    class LearnedBranchPlanner(nn.Module):
        ...
        def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
            hidden = self.net(x)
            return self.mode_head(hidden), self.yield_head(hidden)
        ...
        def predict_context(
            ...
            *,
            ...
            with torch.no_grad():
                mode_logits, yield_score = self.forward(x)
...
def train_branch_planner(
    ...
    *,
    ...
    for _ in range(epochs):
        mode_logits, yield_score = model(X)
        loss = loss_mode(mode_logits, y_mode) + loss_yield(yield_score, y_yield)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
