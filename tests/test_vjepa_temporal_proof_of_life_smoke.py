...
def test_vjepa_temporal_proof_of_life_emits_typed_artifacts(tmp_path) -> None:
    subprocess.run(
        [
            sys.executable,
            "scripts/smoke_test_vjepa_temporal_seam.py",
            "--steps",
            "30",
            "--artifact-dir",
            str(tmp_path),
    ...
    summary = json.loads(
        (tmp_path / "vjepa_temporal_seam_proof_of_life.json").read_text(
            encoding="utf-8"
        )
    )
    metric_report = json.loads(
        (tmp_path / "vjepa_temporal_metric_report.json").read_text(encoding="utf-8")
    )
    benchmark_evidence = json.loads(
        (tmp_path / "vjepa_temporal_benchmark_evidence.json").read_text(
            encoding="utf-8"
        )
    )
    manifest = json.loads(
    ...
    assert summary["seam_type"] == "vjepa_temporal_alignment"
    ...
    assert metric_report["seam_type"] == "vjepa_temporal_alignment"
    ...
    assert benchmark_evidence["subsystem_key"] == "vjepa_temporal_alignment"
    ...
    assert manifest["training_kind"] == "perception_vjepa_temporal_proof_of_life"
    ...
    assert (
        ...
        == "mock_lerobot_droid_vjepa_temporal"
    )
...
def test_vjepa_temporal_proof_of_life_accepts_local_lerobot_rows(tmp_path) -> None:
    ...
    summary = json.loads(
        (tmp_path / "artifacts" / "vjepa_temporal_seam_proof_of_life.json").read_text(
            encoding="utf-8"
        )
    )
    manifest = json.loads(
    ...
    assert summary["data"]["dataset_kind"] == "local_lerobot_rows_vjepa_temporal"
    assert summary["loss_proof"]["loss_decreased"] is True
