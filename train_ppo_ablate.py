"""
PPO Training with Ablation Modes
...
Usage:
    python train_ppo_ablate.py A  # Baseline
...
def run_ablation(mode, episodes=300, use_condition_vector_for_policy: bool = False):
    ...
    if mode == 'A':
        ...
        use_novelty = False
        use_constraint = True
        log_path = 'logs/ablation_A_baseline.csv'
        print("=== ABLATION A: Baseline (weights=1) ===")
    elif mode == 'B':
        # No constraint
        use_novelty = True
        use_constraint = False
        log_path = 'logs/ablation_B_no_lambda.csv'
        print("=== ABLATION B: No constraint (λ=0) ===")
    elif mode == 'C':
        # Full model
        use_novelty = True
        use_constraint = True
        log_path = 'logs/ablation_C_full.csv'
    ...
    # PPO Agent
    # State: [t, completed, attempts, errors] = 4 dims
    # Action: scalar speed in [0, 1] = 1 dim
    state_dim = 4
    action_dim = 1
    ...
    for ep in range(episodes):
        ...
        # Print progress
        if (ep + 1) % 20 == 0:
            print(f"[ep {ep+1:3d}] MP_r={mp_r:.0f}/h  Profit=${profit:.2f}  "
                  f"Err={err_rate:.3f}  λ={lam:.3f}  "
                  f"Wt={train_metrics.get('mean_weight', 1.0):.2f}  "
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="PPO ablation runner")
